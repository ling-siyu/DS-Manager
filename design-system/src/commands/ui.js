...
import { buildProjectCss, buildSecuramarkCss } from '../utils/securamark-css.js';
...
export async function createPreviewServer(paths = resolveProjectPaths(), options = {}) {
  ...
  // Compile the project's Tailwind utilities so its real components render styled
  // inside the preview (their /@fs sources reference Tailwind classes + --ds vars).
  const compileProjectCss = async (data) => {
    if (!paths.repoRoot) return;
    try {
      // Prefer compiling the project's real global stylesheet (full fidelity);
      // fall back to a synthesized utilities-only sheet if it has none.
      let css = await buildProjectCss({ projectDir: paths.repoRoot });
      if (!css) {
        css = await buildSecuramarkCss({
          securamarkDir: paths.repoRoot,
          tokensPath: paths.tokensPath,
        });
      }
      data.projectCss = css;
    } catch (error) {
      onLog(`Project CSS compile skipped: ${error.message}`);
    }
  };

  let current = buildPreviewData(paths);
  await compileProjectCss(current);
  ...
  const server = await createViteServer({
    ...
    // Single React copy even when rendering the project's cross-file components.
    resolve: { dedupe: ['react', 'react-dom'] },
    server: {
      host,
      port,
      strictPort: true,
      // Allow fs access to: the Vite preview root itself (ships inside the dsm
      // package — under node_modules when DSM is installed into a target, so it
      // is NOT covered by dsRoot) and the whole target repo (so component
      // sources loaded via /@fs are serveable).
      fs: { allow: [PREVIEW_DIR, paths.repoRoot, paths.dsRoot].filter(Boolean) },
    },
    clearScreen: false,
    logLevel: 'warn',
  });
...
export async function uiCommand(options = {}) {
  ...
  console.log(chalk.dim(
    `  ${data.tokens.length} tokens · ${data.components.length} components`,
  ));
